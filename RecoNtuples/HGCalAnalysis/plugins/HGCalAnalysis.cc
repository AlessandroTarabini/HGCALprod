...
class HGCalAnalysis : public edm::one::EDAnalyzer<edm::one::WatchRuns, edm::one::SharedResources> {
 ...
 private:
  ...
  std::vector<float> cluster2d_t_;
  std::vector<float> cluster2d_dt_;
...
//      tok_geom2_(esConsumes<CaloGeometry, CaloGeometryRecord>()),
  ...
  hgcalLayerClusterTimeToken_ = consumes<edm::ValueMap<std::pair<float, float>>>(edm::InputTag("hgcalLayerClusters" , "timeLayerCluster" , "TEST"));
  ...
  if (storePFCandidates_) {
    pfCandidates_ = consumes<std::vector<reco::PFCandidate>>(edm::InputTag("particleFlow","",inputTag_Reco_));
  }

  //  pfClusters_ = consumes<std::vector<reco::PFCluster>>(edm::InputTag("particleFlowClusterHGCal","",inputTag_Reco_));
  ...
  */
  ...
  multiClusters_ =
    consumes<std::vector<reco::HGCalMultiCluster>>(edm::InputTag(inputTag_HGCalMultiCluster_,"",inputTag_ReReco_));
  tracks_ = consumes<std::vector<reco::Track>>(edm::InputTag("generalTracks"));
  electrons_ =
    consumes<std::vector<reco::GsfElectron>>(edm::InputTag("ecalDrivenGsfElectronsHGC","",inputTag_Reco_));
  electrons_ValueMapClusters_ = consumes<edm::ValueMap<reco::CaloClusterPtr>>(
									      edm::InputTag("particleFlowSuperClusterHGCal","PFClusterAssociationEBEE",inputTag_Reco_));
  ...
  if (storeMoreGenInfo_) {
    ...
    t_->Branch("genpart_ovt", &genpart_ovt_);
  ...
  t_->Branch("cluster2d_t", &cluster2d_t_);
  t_->Branch("cluster2d_dt", &cluster2d_dt_);
  ...
  t_->Branch("multiclus_time", &multiclus_t_);
...
void HGCalAnalysis::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  //  if(multiClusters.size()<9999){
  if(multiClusters.size() > 0){
        std::cout<<"number of multiclusters =>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> "<<multiClusters.size()<<std::endl;
  ...
  for (unsigned int i = 0; i < npart; ++i) {
    ...
    double fbrem = -1;
    // std::cout<<"track# : "<<i<<" z pos: "<<std::abs(myTrack.vertex().position().z())<<std::endl;
    ...
    genpart_ovt_.push_back(myTrack.vertex().position().t());
  ...
  */
  ...
  //  std::cout<<"lcTime.size() = "<<lcTime.size()<<std::endl;

  //  for (unsigned ict = 0; ict < lcTime.size(); ++ict) {
    //    std::cout<<"lcTime val = "<<ic.first<<"  "<<ic.second<<std::endl;
    // std::cout<<"lcTime val = "<<lcTime[]<<std::endl;
  ...
  for (unsigned ic = 0; ic < nclus; ++ic) {
    ...
    if (timeClusterKey.first > 0){
      ...
      //      std::cout<<"pt: "<<clusterPtr->energy() / cosh(clusterPtr->eta());
      std::cout<<" energy: "<<clusterPtr->energy()<<std::endl;
      const std::vector<std::pair<DetId, float>> &hf = clusterPtr->hitsAndFractions();
      std::cout<<"hit size: "<<hf.size()<<std::endl;
    }
  }
  */
  ...
  if (algo_ < 4){
    ...
    for (unsigned int i = 0; i < multiClusters.size(); i++) {
      ...
      //      std::cout<<"start filling multiclister:"<<i<<std::endl;
	...
	//	cout<<"layer: "<<layer<<std::endl;
      ...
      multiclus_t_.push_back(multiClusters[i].time());
      ...
      multiclus_slopeY_.push_back(multiClusters[i].y());
	    ...
	    //      std::cout<<"working filling multiclister branches"<<std::endl;
  ...
  //  std::cout<<"okay till here============================================================>3"<<std::endl;
  if (readGen_) {
 ...
 /*
  if (algo_ < 4){
    // loop over pfClusters From MultiClusters (python label particleFlowClusterHGCalFromMultiCl)
        removing since this doesnt exist anymore
...
*/
    //  std::cout<<"okay till here============================================================>5"<<std::endl;
  ...
  //  std::cout<<"okay till here============================================================>6"<<std::endl;
  // loop over tracks
  ...
  //   std::cout<<"okay till here============================================================>7"<<std::endl;
  if (storePFCandidates_) {
...
int HGCalAnalysis::fillLayerCluster(const edm::Ptr<reco::CaloCluster> &layerCluster,
  ...
  //  std::cout<<"multiClusterIndex: "<<multiClusterIndex<<std::endl;
  ...
  for (unsigned int j = 0; j < hfsize; j++) {
    ...
    //    std::cout<<"layer|detid :"<<layer<<"|"<<int(rh_detid)<<std::endl;
    if (fillRecHits) {
      if (storedRecHits_.find(rh_detid) == storedRecHits_.end()) {
        ...
        fillRecHit(rh_detid, fraction, layer, cluster_index_); //changed
	//	storedRecHits_.insert(rh_detid); // added for rechit fix
      } else {
  ...
  //  std::cout << "in fillLayerCluster2................................>" << std::endl;
  double pt = layerCluster->energy() / cosh(layerCluster->eta());
  //  if (layerCluster->z() > 400)  std::cout<<"layer : z :"<<layer<<"  "<<layerCluster->z()<<std::endl;
  ...
  //  cluster2d_t_.push_back(layerCluster->t());
...
void HGCalAnalysis::fillRecHit(const DetId &detid, const float &fraction, const unsigned int &layer,
  ...
  storedRecHits_.insert(detid); //////added for debug to retain_common_rechits
